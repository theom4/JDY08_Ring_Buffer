...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  rssi_value = atoi(buffer_to_copy);
	  //snprintf((char*)rssi_uart,7,"s:%d",rssi_value);
	  distance = rssi_get_distance(n_factor, rssi_value);
	  snprintf((char*)rssi_uart,15,"dist:%fm",distance);
	  HAL_UART_Transmit(&huart2, rssi_uart, 9,100);
    /*if(is_response("ledblue"))
     * {
     * 		HAL_GPIO_WritePin(GPIOB,BLUE_LED,1);
     * }
     * if(is_response("ledoff"))
     * {
